...
% Input: 
% - EEG structure
% - hp: high frequency cut-off [Hz]; default [1]; order filter is set to 3
% - lp: low frequency cut-off [Hz]; default [80]; order filter is set to 3
...
% --------------------------
% Author: Fecchio Matteo, Adenauer Girardi Casali, Silvia Casarotto
% --------------------------

function [EEG]=EEGfilters(EEG,hp,lp,bs1,bs2,dws)
if isempty(hp)
    hp=0;
end
if isempty(lp)
...
%%%% CHECK - downsample also EEG.epoch.latency
if strcmp(dws,'Y') && EEG.srate~=256
    NewRate=256;
    if ~isfield(EEG,'filters') || (2*EEG.filters.lowpass/EEG.srate)>1/(EEG.srate/NewRate)
        [Bl,Al]=butter(3,1/(EEG.srate/NewRate),'low');
        check=true;
    else
        check=false;
    end
    ...
    if length(size(EEG.data))~=2
        ...
        if check
            datatmp=squeeze(num2cell(EEG.data,[1 2]))';
            datatmp=cellfun(@(x) filtfilt(Bl,Al,double(x'))',datatmp,'UniformOutput', false);
            EEG.data=reshape(cell2mat(datatmp),size(datatmp{1},1),size(datatmp{1},2),size(datatmp,2));
            clear datatmp
        end
        
        for kk=1:size(EEG.data,1)
            ch=squeeze(num2cell(EEG.data(kk,:,:),[1 2]))';
            ch=cellfun(@(x) interp1(EEG.times,x,timesOUT,'spline'),ch,'UniformOutput', false);
            datatmp(kk,:,:)=reshape(cell2mat(ch),size(ch{1},2),size(ch,2));
        end
        
    else
        if check
            EEG.data=filtfilt(Bl,Al,EEG.data);
        end
        for kk=1:size(EEG.data,1)
            ch=EEG.data(kk,:);
            datatmp(kk,:)=interp1(EEG.times,ch,timesOUT,'spline');
        end
    end
    EEG.times=timesOUT;
    EEG.srate=NewRate;
