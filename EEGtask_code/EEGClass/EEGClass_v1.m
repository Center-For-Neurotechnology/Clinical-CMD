...
% Function will figure out n_k based on # trials in data.
% if ~exist('n_k', 'var') || nargin<7
%     n_k = []; % Number of splits (approximate number of trials)
% end
% % Figure out kfold value from number of trials if this hasn't been supplied already.
% if isempty(n_k)
%     if ~strcmp(task(1:5), 'Motor')
%         if (split_task && split_trl)
%             n_k = 2*(size(data,1)/trl_sec);
%         elseif ~split_task && ~split_trl || (split_task && ~split_trl)
%             n_k = size(data,1)/trl_sec;
%         elseif ~split_task && split_trl
%             n_k = 2*size(data,1)/trl_sec;
%         end
%     end
% end
...
title(hfig,['pvalue:' num2str(round(class.p_value*1000)/1000) ' - class accuracy:' num2str(round(class.accuracy*100)/100)])
