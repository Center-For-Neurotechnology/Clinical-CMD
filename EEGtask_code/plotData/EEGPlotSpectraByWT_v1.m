% -----------------------------------------------------------------------
% Input: 
% - specON: PSD of ON (active) blocks [freqs x epochs x channels]
% - specOFF: PSD of OFF (rest) blocks [freqs x epochs x channels]
% - class: class ctructure with SVM info
...
% --------------------------
function EEGPlotSpectraByWT_v1(specON,specOFF,class,chanlocs,f)
...
for jj=1:4
    ...
    topoplot(abs(class.pooled_wt_avg_freq(jj,:)),chanlocs,...
        'maplimits',[-0.3 0.3],'electrodes','ptslabels','conv','off');
    title(['Weights in the ' freq_names{jj} ' band'])
end
