% -----------------------------------------------------------------------
% Input: 
% - spectraON: PSD of ON (active) blocks [freqs x epochs x channels]
% - spectraOFF: PSD of OFF (rest) blocks [freqs x epochs x channels]
% - f: vector of frequencies
% - chanlocs: channels location (EEG chanlocs structure)
% - goodCH: vector of good channels
...
freq_bands_lim = {[1,3],[4,7],[8,13],[14,30]};
freq_bands=cellfun(@(x) find(abs(f-x(1))==min(abs(f-x(1)))): ...
    find(abs(f-x(2))==min(abs(f-x(2)))),freq_bands_lim,'UniformOutput',false);
freq_names={'Delta','Theta','Alpha','Beta'};
X=[1 freq_bands_lim{2}(1) freq_bands_lim{3}(1) freq_bands_lim{4}(1) freq_bands_lim{4}(2)];
...
FCPOelecNames={'PSD Frontal','PSD Central','PSD Parietal','PSD Occipital'};
LMR={left_elecs, middle_elecs, right_elecs};
LMRelecNames={'PSD Left','PSD Midline','PSD Right'};

switch fig_type
    case 'DisplayAllPSD'
%       Display of the PSD averaged across epochs - ON (active) and OFF (rest) blocks
        EEGDisplayAllPSD(spectrum_avg{1}',spectrum_avg{2}',f,chanlocs(goodCH))
        
%       Display of the PSD averaged across epochs and channels 
%       - ON blocks (active) in RED, and OFF blocks (rest) in blue
        figure('NumberTitle','off','Name','PSD Averaged across all electrodes');
        ...
        title('PSD Averaged across all electrodes', 'FontWeight', 'Bold', 'Fontsize', 12)
        set(gca, 'xlim', freq_range, 'xtick',X , 'xticklabel', X, 'fontsize', 8)
        xlabel(gca, 'Hz', 'FontWeight', 'Bold', 'Fontsize', 10)
        ylabel(gca, 'Power (dB)', 'FontWeight', 'Bold', 'Fontsize', 10)
        axis(gca, 'square')
        legend({'Task ON','Task OFF'})
        Y=ylim;
        line(gca,[freq_bands{2}(1) freq_bands{2}(1)],[Y(1) Y(2)],'Color','k','LineStyle','--','LineWidth', 1.5)
        line(gca,[freq_bands{3}(1) freq_bands{3}(1)],[Y(1) Y(2)],'Color','k','LineStyle','--','LineWidth', 1.5)
        line(gca,[freq_bands{4}(1) freq_bands{4}(1)],[Y(1) Y(2)],'Color','k','LineStyle','--','LineWidth', 1.5)
    ...
    case 'PSD_F_C_P_O'
        hfig=figure('NumberTitle','off','Name',...
            'PSD averaged across Frontal, Central, Posterior, and Occipital electrodes');
        Y=[];
        for kk=1:length(FCPO)
            ...
            title(FCPOelecNames{kk}, 'FontWeight', 'Bold', 'Fontsize', 8)
            Y(kk,:)=ylim;
            legend({'Task ON','Task OFF'})
        end
        
        axH = findall(gcf,'type','axes');
        set(axH, 'xlim', freq_range, 'ylim', [min(Y(:,1)) max(Y(:,2))], 'xtick', X, 'xticklabel', X, 'fontsize', 8)
        for j = 1:length(axH)
    ...
    case 'PSD_L_M_R'
        hfig=figure('NumberTitle','off','Name',...
            'PSD averaged across Left, Midline, and Right electrodes');
        Y=[];
        for kk=1:length(LMR)
            ...
            Y(kk,:)=ylim;
            title(LMRelecNames{kk}, 'FontWeight', 'Bold', 'Fontsize', 8)
        end
        axH = findall(gcf,'type','axes');
        set(axH, 'xlim', freq_range, 'ylim', [min(Y(:,1)) max(Y(:,2))], 'xtick', X, 'xticklabel', X, 'fontsize', 8)
        for j = 1:length(axH)
        ...
        for kk=1:length(LMR)
            subplot(2,3,kk+3),
            LMR_bar = bar(LMR_M{kk}); title(LMRelecNames{kk}, 'FontWeight', 'Bold', 'Fontsize', 8);
            hold on
        ...
        set(axH,'xlim', [0.5 4.5], 'xticklabel', {'d', 't', 'a', 'b',}, 'fontsize', 8, 'fontweight', 'bold', ...
        ...
        for kk=1:length(LMR)
            ...
            set(gca, 'xlim', freq_range, 'ylim', [min(Y(:,1)) max(Y(:,2))], 'xtick', X, 'xticklabel', X, 'fontsize', 8)
            xlabel(gca, 'Hz', 'FontWeight', 'Bold', 'Fontsize', 7)
            ylabel(gca, 'Power (dB)', 'FontWeight', 'Bold', 'Fontsize', 7)
            %         ylim([min(min(Y)) 10])
    ...
    case 'PSD_Bar_L_M_R'
        ...
        for kk=1:length(LMR)
            subplot(1,3,kk),
            LMR_bar = bar(LMR_M{kk}); 
            title(LMRelecNames{kk}, 'FontWeight', 'Bold', 'Fontsize', 8);
            set(LMR_bar(1,1), 'FaceColor', 'red');
            set(LMR_bar(1,2), 'FaceColor', 'blue');
            hold on
            for b = 1:nbars
