% -----------------------------------------------------------------------
% Graphical User Interface that allows to select ICA components
...
% Last Modified by GUIDE v2.5 20-Apr-2019 13:43:32
...
handles.EEG.goodchannels=setdiff(1:handles.EEG.nbchan,handles.EEG.badchannels);
handles.index_ch2remove=[];

set(handles.ICscroll,'enable','off')
set(handles.pushbutton10,'enable','off')
set(handles.pushAvgAct,'enable','off')
set(handles.pruned_ICA,'enable','off')
set(handles.pushbutton9_forward,'enable','off')
set(handles.pushbutton8_back,'enable','off')
set(handles.rem,'enable','off')
set(handles.keep,'enable','off')
...
plot(handles.EEG.times,mean(compproj,3)','b','LineWidth',1)%,xlim([-600 600])
ylabel('\muV')
...
% if isempty(tmp1)
%     title(handles.current_bfly,'Original Data')
% else
    title(handles.current_bfly,['Original Data - comps. to remove (',num2str(varegg*100,'%2.2f'),'%)'])
% end
...
if ~isempty(get(handles.listbox_remICs,'String'))
    ...
    if ~isempty(handles.index_ch2remove)
        ...
        set(handles.listbox_remICs,'Value',[]);
...
alreadyrem=str2num(tmp);
    ...
    plot(handles.EEG.times,mean(compproj,3)','b','LineWidth',1),ylim(tmpy)%,xlim([-600 600]);
    ylabel('\muV')
...
%     set(handles.residual_var,'string',strcat('variance:',num2str(varegg*100)));
    ...
    if isempty(tmp)
        title(handles.proposed_bfly,['Original Data - N',num2str(selcomp),' (',num2str(varegg*100,'%2.2f'),'%)'])
    else
        title(handles.proposed_bfly,['Original Data - comps. to remove - N',num2str(selcomp),' (',num2str(varegg*100,'%2.2f'),'%)'])
    end
...
he=plot(freqs(1:a),spectra(1:a),'-r');
set(he,'LineWidth',2)
set(gca, 'xlim', [0 min(freqslim, handles.EEG.srate/2)]);
...
axes(handles.ICtopo)
cla(handles.ICtopo)
topoplot((handles.EEG.icawinv(handles.EEG.goodchannels,selcomp)),handles.EEG.chanlocs(handles.EEG.goodchannels),'electrodes','on','maplimits','maxmin'),caxis([-max(abs(handles.EEG.icawinv(:,selcomp))) max(abs(handles.EEG.icawinv(:,selcomp)))]) ;
...
tmptoplot=permute(handles.EEG.icaact(selcomp,:,:),[3,2,1]);
plot(handles.EEG.times,mean(tmptoplot,1),'b')%,xlim([-600 600]);
ylabel('\muV')
xlabel('Time(ms)')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% if size(handles.EEG.data,1)>length(handles.EEG.goodchannels)
%     handles.EEG.data=handles.EEG.data(handles.EEG.goodchannels,:,:);
% else
%     handles.EEG.data=handles.EEG.data;
% end
% if size(handles.EEG.data,3)>length(handles.EEG.goodtr)
%     handles.EEG.data=handles.EEG.data(:,:,handles.EEG.goodtr);
% else
%     handles.EEG.data=handles.EEG.data;
% end
% handles.EEG.nbchan=length(handles.EEG.goodchannels);
% handles.EEG.data=handles.EEG.data(handles.EEG.goodchannels,:,handles.EEG.goodtr);
% handles.EEG.nbchan=length(handles.EEG.goodchannels);
...
ylabel('\muV')       
if isempty(tmp1)
    title(handles.proposed_bfly,['Original Data - N',num2str(tmp2),' (',num2str(varegg*100,'%2.2f'),'%)'])
else
    title(handles.proposed_bfly,['Original Data - comps. to remove - N',num2str(tmp2),' (',num2str(varegg*100,'%2.2f'),'%)'])
end
...
camp_butterfly=get(handles.ICs_list,'Value');
axes(handles.ICtopo)
topoplot((handles.EEG.icawinv(handles.EEG.goodchannels,camp_butterfly)),handles.EEG.chanlocs(handles.EEG.goodchannels),'electrodes','on','maplimits','maxmin'),caxis([-max(abs(handles.EEG.icawinv(:,camp_butterfly))) max(abs(handles.EEG.icawinv(:,camp_butterfly)))]);%,colorbar ;
...
set(handles.pushAvgAct,'enable','on')
set(handles.pruned_ICA,'enable','on')
set(handles.pushbutton9_forward,'enable','on')
set(handles.pushbutton8_back,'enable','on')
set(handles.rem,'enable','on')
set(handles.keep,'enable','on')
...
eegplot( handles.EEG.icaact, 'srate', handles.EEG.srate, 'title', 'Scroll component activities -- eegplot()', ...
    'limits', [handles.EEG.xmin handles.EEG.xmax]*1000);
...
prompt = {'Select components (for example 1:20)'};
dlg_title = 'Components';
num_lines = 1;
def = {''};
tmp_comp = str2num(str2mat(inputdlg(prompt,dlg_title,num_lines,def)));
clear def num_lines dlg_title prompt

if length(tmp_comp)>1
dim_fig(1)=round(sqrt(length(tmp_comp)));
dim_fig(2)=ceil(length(tmp_comp)/dim_fig(1));

h = waitbar(0,' waiting ...');
figure
for kk=1:length(tmp_comp)
    waitbar(kk/length(tmp_comp));
    subplot(dim_fig(1),dim_fig(2),kk)
    topoplot((handles.EEG.icawinv(handles.EEG.goodchannels,tmp_comp(kk))),handles.EEG.chanlocs(handles.EEG.goodchannels),'electrodes','on','maplimits','maxmin'),caxis([-max(abs(handles.EEG.icawinv(:,tmp_comp(kk)))) max(abs(handles.EEG.icawinv(:,tmp_comp(kk))))]);%,colorbar ;
    title(num2str(tmp_comp(kk)))
end
close(h)

elseif length(tmp_comp)==1
    figure, topoplot((handles.EEG.icawinv(handles.EEG.goodchannels,tmp_comp)),handles.EEG.chanlocs(handles.EEG.goodchannels),'electrodes','ptslabels','maplimits','maxmin','conv','on'),caxis([-max(abs(handles.EEG.icawinv(:,tmp_comp))) max(abs(handles.EEG.icawinv(:,tmp_comp)))]);%,colorbar ;
    title(num2str(tmp_comp))
...
% --- Executes on button press in pushAvgAct.
function pushAvgAct_Callback(hObject, eventdata, handles)
dim_fig(1)=round(sqrt(size(handles.EEG.icaact,1)));
dim_fig(2)=ceil(size(handles.EEG.icaact,1)/dim_fig(1));

h = waitbar(0,' waiting ...');
figure
for selcomp=1:size(handles.EEG.icaact,1)
    waitbar(selcomp/size(handles.EEG.icaact,1));
    subplot(dim_fig(1),dim_fig(2),selcomp)
    tmptoplot=permute(handles.EEG.icaact(selcomp,:,:),[3,2,1]);
    plot(handles.EEG.times,mean(tmptoplot,1),'b')
    xlim([handles.EEG.times(1) handles.EEG.times(end)]);
    xticklabels('')
    yticklabels('')
...
% hObject    handle to pushAvgAct (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
